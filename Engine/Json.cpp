...
namespace json {
    bool Load(const std::string& filename, document_t& document) {
        ...
        std::string buffer;
        if (!fe::ReadTextFile(filename, buffer)) {
    ...
    bool Read(const value_t& value, const std::string& name, nu::Vector2& data, bool required) {
    ...
    bool Read(const value_t& value, const std::string& name, nu::Vector3& data, bool required) {
